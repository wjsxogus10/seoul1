...
# 1. 페이지 설정
# --------------------------------------------------------------------------
st.set_page_config(layout="wide", page_title="서울시 대중교통 개선 대시보드 (지하철 중심)")
...
def load_and_merge_data():
    ...
    # [B] 초기화 (버스 컬럼 부활)
    cols_init = ['총_상주인구_수', '인구 밀도', '버스정류장_수', '지하철역_수', '총_교통수단_수']
    ...
    if bus_path:
        try:
            ...
            if '자치구' in df_bus.columns:
                ...
                data_info['버스'] = {'file': target_bus_file, 'status': '✅ (로드됨)'}
            else:
...
if page_mode == "🏙️ 서울시 대중교통 현황":
    st.title("🏙️ 서울시 대중교통 현황 (지하철 중심)")
    
    st.sidebar.header("🔍 분석 옵션")
    
    # [분석 옵션] 여기서는 버스 제외 (순위 계산에서 빠졌으므로)
    available_cols = [
    ...
    if valid_metrics:
        ...
        # 지도 Hover에서도 버스 제외 (분석 화면이니까)
        hover_cols = ['자치구명', '총 상주인구 수 (명)', '면적 (km²)', '총 교통수단 수', '지하철역 수 (개)']
...
# --------------------------------------------------------------------------
    ...
    # Hover에만 표시해 줄까 하다가, 일관성을 위해 여기도 버스는 뺌
    scat_fig = px.scatter(
        gdf, 
        x='인구 밀도 (명/km²)', 
        y='대중교통 수 (인구 천명당)', 
...
# --------------------------------------------------------------------------
    ...
    try:
        ...
        subway_val = target_row.get('지하철역 수 (개)', 0)
        bus_val = target_row.get('버스정류장 수 (개)', 0) # 여기서 가져옴
        rank_val = target_row.get('교통 부족 순위 (위)', 0)
        
        with c1: st.metric("👥 총 인구", f"{pop_val:,.0f}명")
        with c2: st.metric("🗺️ 면적", f"{area_val:.2f}km²")
        with c3: st.metric("🚌 버스정류장", f"{bus_val:,.0f}개") # 버스 표시
        with c4: st.metric("🚇 지하철역", f"{subway_val:,.0f}개")
        
        st.markdown(f"##### 🚨 **교통 부족 순위: {rank_val:.0f}위** (지하철 기준)")
        st.markdown("---")
        ...
        with col_d1:
            ...
            fig_d.update_traces(
                ...
                "🚌 버스: %{customdata[2]:,.0f}개<br>" +
                "🚇 지하철: %{customdata[3]:,.0f}개<br>" +
                "🚨 순위: %{customdata[4]:.0f}위<extra></extra>"
            )
