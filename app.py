...
def load_and_merge_data():
    ...
    try:
        ...
        if os.path.exists(bus_path):
            ...
            if success:
                cnt = df_bus_stations.groupby('자치구명').size().reset_index(name='버스_cnt')
    ...
    try:
        ...
        if os.path.exists(path):
            ...
            gu = next((c for c in df.columns if '자치구' in c), None)
            den = next((c for c in df.columns if '밀도' in c), None)
            cnt = next((c for c in df.columns if '역' in c and '수' in c), None)
            
            if gu and den:
                rename_map = {gu: '자치구명', den: '지하철역 밀도'}
                cols = ['자치구명', '지하철역 밀도']
                if cnt: 
                    rename_map[cnt] = '지하철역_수'
                    cols.append('지하철역_수')
                
                df = df.rename(columns=rename_map)
                gdf = gdf.merge(df[cols], on='자치구명', how='left', suffixes=('', '_sub'))
