...
def load_and_merge_data():
    ...
    try:
        ...
        # [삭제] 지도 데이터에 포함된 base_year 삭제
        if 'base_year' in gdf.columns:
            gdf = gdf.drop(columns=['base_year'])
...
if page_mode == "🏙️ 서울시 대중교통 현황":
    ...
    if '버스정류장 수 (개)' in gdf.columns:
        metrics_order.insert(0, '버스정류장 수 (개)')
