...
def load_and_merge_data():
    ...
    if os.path.exists(pop_file):
        try:
            ...
            gdf['인구_밀도'] = gdf['총_상주인구_수'] / gdf['면적(km²)']
        except: pass
    ...
    if os.path.exists(bus_file):
        try:
            ...
            gdf['버스_밀도'] = gdf['버스정류장_수'] / gdf['면적(km²)']
        except: pass
    ...
    if subway_files:
        try:
            ...
            if x_col and y_col:
                ...
                gdf['지하철_밀도'] = gdf['지하철역_수'] / gdf['면적(km²)']
        except: pass
    else:
        gdf['지하철역_수'] = 0
        gdf['지하철_밀도'] = 0
    ...
    # -----------------------------------------------------------
    # [핵심] 단위 포함해서 이름 깔끔하게 바꾸기
    # -----------------------------------------------------------
    rename_map = {
        '총_상주인구_수': '총 상주인구 수 (명)',  # <--- [단위 추가]
        '집객시설_수': '집객시설 수 (개)',
        '면적(km²)': '면적 (km²)',
        '인구_밀도': '인구 밀도 (명/km²)',
        '버스정류장_수': '버스정류장 수 (개)',
